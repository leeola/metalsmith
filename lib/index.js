 ...
 * @return {Strings} A copy of the internal list.
 */
Metalsmith.prototype.ignore = function(files){
  if (!arguments.length) return this._ignores.slice();
  this._ignores = this._ignores.concat(files);
  return this
}
...
Metalsmith.prototype.read = function(fn){
  var files = {};
  var src = this.source();
  var ignores = this._ignores || null;

  readdir(src, ignores, function(err, arr){
    if (err) return fn(err);
